    ...
    public class ToolsmithModSystem : ModSystem {
        ...
        public override void AssetsFinalize(ICoreAPI api) {
            ...
            if (api.Side.IsClient()) {
                ...
                ToolsmithConstants.ToolsmithHeadItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-head" });
                ToolsmithConstants.ToolsmithHandleItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-handle" });
                ToolsmithConstants.ToolsmithBindingItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-binding" });
                ToolsmithConstants.ToolsmithPartItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-part" });
                ToolsmithConstants.ToolsmithMaintenanceItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-maintenance" });

                ToolsmithConstants.ToolsmithBindingBlockTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-binding" });
                ToolsmithConstants.ToolsmithPartBlockTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-part" });
            ...
            var bindingItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-part", "toolsmith-binding" });
            var bindingBlockTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-part", "toolsmith-binding" });
            var handleItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(new string[] { "toolsmith-part", "toolsmith-handle" });
            ...
            foreach (var t in api.World.Collectibles.Where(t => t?.Code != null)) { //A tool/part should likely be only one of these!
                if (ConfigUtility.IsTinkerableTool(t.Code.ToString()) && !(ConfigUtility.IsToolHead(t.Code.ToString())) && !(ConfigUtility.IsOnBlacklist(t.Code.ToString()))) { //Any tool that you actually craft from a Tool Head to create!
                    ...
                    if (t.ItemClass == EnumItemClass.Item) {
                        var item = ((Item)t);
                        if (!handleItemTag.IsFullyContainedIn(item.Tags))
                        {
                            var tagsList = api.CollectibleTagRegistry.SlowEnumerateTagNames(item.Tags).ToList();
                            tagsList.Add("toolsmith-part");
                            tagsList.Add("toolsmith-handle");
                            item.Tags = api.CollectibleTagRegistry.RegisterAndCreateTagSet(tagsList.Distinct().ToArray());
                        }
                    }
                    ...
                    if (t.ItemClass == EnumItemClass.Item) {
                        var item = ((Item)t);
                        if (!bindingItemTag.IsFullyContainedIn(item.Tags))
                        {
                            var tagsList = api.CollectibleTagRegistry.SlowEnumerateTagNames(item.Tags).ToList();
                            tagsList.Add("toolsmith-part");
                            tagsList.Add("toolsmith-binding");
                            item.Tags = api.CollectibleTagRegistry.RegisterAndCreateTagSet(tagsList.Distinct().ToArray());
                        }
                    } else {
                        var block = ((Block)t);
                        if (!bindingBlockTag.IsFullyContainedIn(block.Tags))
                        {
                            var tagsList = api.CollectibleTagRegistry.SlowEnumerateTagNames(block.Tags).ToList();
                            tagsList.Add("toolsmith-part");
                            tagsList.Add("toolsmith-binding");
                            block.Tags = api.CollectibleTagRegistry.RegisterAndCreateTagSet(tagsList.Distinct().ToArray());
                        }
                    }
