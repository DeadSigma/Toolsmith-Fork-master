...
namespace Toolsmith.ToolTinkering {
    ...
    public class ToolTinkeringPatches {
        ...
        [HarmonyPrefix]
        [HarmonyPatch(nameof(CollectibleObject.DamageItem)), HarmonyPriority(Priority.High)]
        private static bool TinkeredToolDamageItemPrefix(IWorldAccessor world, Entity byEntity, ItemSlot itemSlot, int amount, bool destroyOnZeroDurability, CollectibleObject __instance)
        {
            // Замени все itemSlot на itemSlot внутри этого блока!
            // Например: if (world.Side.IsServer() && !itemSlot.Itemstack.GetBrokeWhileSharpeningFlag()...)
                       ...
                       while (j > 0) {
                           if (codes[j].opcode == OpCodes.Ldloc_0)
                           {
                               ...
                               while (k > 0)
                               {
                                   ...
                                   {
                                       // Теперь мы копируем и команду, и её операнд целиком!
                                       codes[j].opcode = codes[k - 1].opcode;
                                       codes[j].operand = codes[k - 1].operand;
                                       break;
                                   }
                                   k--;
                               }
