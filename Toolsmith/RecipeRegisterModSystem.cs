...
namespace Toolsmith {
    public class RecipeRegisterModSystem : ModSystem {
        ...
        public override void AssetsFinalize(ICoreAPI api) {
            ...
            var headItemTag = api.CollectibleTagRegistry.RegisterAndCreateTagSet(["toolsmith-part", "toolsmith-head"]);
            ...
            foreach (var recipe in api.World.GridRecipes) { //Check each recipe...
                foreach (var tool in TinkerableToolsList.Where(t => recipe.Output.Code.Equals(t.Code))) { //Where the output code matches anything on the Tinkered Tool List (from the configs)...
                    foreach (var ingredient in recipe.ResolvedIngredients.Where(i => (i != null) && (i.ResolvedItemStack != null) && (ConfigUtility.IsToolHead(i.ResolvedItemStack.Collectible?.Code.ToString())))) { //And the recipe in question has a Tool Head item that is on the Tool Head Config List
                        ...
                        if (ingredient.ResolvedItemStack.Item != null && !headItemTag.IsFullyContainedIn(ingredient.ResolvedItemStack.Item.Tags))
                        {
                            // 1. Достаем старые теги в виде списка строк
                            var tagsList = api.CollectibleTagRegistry.SlowEnumerateTagNames(ingredient.ResolvedItemStack.Item.Tags).ToList();

                            // 2. Добавляем наши новые теги
                            tagsList.Add("toolsmith-part");
                            tagsList.Add("toolsmith-head");

                            // 3. Убираем дубликаты (Distinct) и создаем новый TagSet через реестр
                            ingredient.ResolvedItemStack.Item.Tags = api.CollectibleTagRegistry.RegisterAndCreateTagSet(tagsList.Distinct().ToArray());
                        }
        ...
        private List<GridRecipe> GenerateToolGridRecipes(ICoreAPI api, CraftingRecipeIngredient head, CollectibleObject tool) {
            ...
            foreach (var handle in HandleList) {
                foreach (var binding in BindingList) {
                    ...
                    recipe.Resolve(api.World, "Toolsmith Mod Recipe");
                    list.Add(recipe);
                }
            }
                        ...
                        foreach (var treatmentMat in TreatmentList) {
                            ...
                            if (treatmentStats != null && !treatmentStats.isLiquid) {
                                ...
                                foreach (var container in LiquidContainers) {
                                    ...
                                    bucketRecipe.Resolve(api.World, "Toolsmith Mod Recipe");
        ...
        private List<GridRecipe> GenerateSandpaperRecipes(ICoreAPI api) { //TODO: Rework this eventually to pull from the config files again. Guh. I don't really want to go that far right now.
            ...
            foreach (var container in LiquidContainers) {
                ...
                containerRecipe.Resolve(api.World, "Toolsmith Mod Recipe");
                ...
                /*if (butcheringModEnabled) {
                    ...
                    containerSinewRecipe.Resolve(api.World);
                    containerHideRecipe.Resolve(api.World);
